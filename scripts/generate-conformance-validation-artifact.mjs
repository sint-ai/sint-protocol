...
const rawVitestPath = path.join(reportDir, "conformance-validation-vitest.json");
const artifactJsonPath = path.join(reportDir, "conformance-validation-artifact.json");
const artifactMdPath = path.join(reportDir, "conformance-validation-artifact.md");
...
console.log("[conformance] running conformance validation...");
...
const suites = (raw.testResults ?? []).map((suite) => {
  ...
  return {
    file: path.relative(repoRoot, suite.name ?? suite.file ?? "unknown"),
...
const payload = {
  artifactVersion: "v1",
  artifactType: "conformance-validation",
  generatedAt,
  gitSha,
...
console.log(`[conformance] wrote ${path.relative(repoRoot, artifactJsonPath)}`);
console.log(`[conformance] wrote ${path.relative(repoRoot, artifactMdPath)}`);
