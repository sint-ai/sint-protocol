...
export type ApprovalResolution =
  | { readonly status: "approved"; readonly by: string; readonly at: ISO8601 }
...
export class ApprovalQueue {
   ...
   */
  enqueue(request: SintRequest, decision: PolicyDecision): ApprovalRequest {
    ...
    this.pending.set(request.requestId, { request: approvalRequest, timer });
   ...
   * Cancels the timeout timer if still running.
   */
  resolve(
    requestId: string,
    ...
    const resolved: ApprovalResolution =
      resolution.status === "approved"
        ? { status: "approved", by: resolution.by, at: nowISO8601() }
        : { status: "denied", by: resolution.by, reason: resolution.reason ?? "Denied by reviewer" };
